...
class ParserStream extends Readable {
  constructor (input: stream, { baseIRI = '', context = null, factory = rdf } = {}) {
    super({
      objectMode: true,
      read: () => {},
    });
    ...
    const concatStream = concat({ encoding: 'string' }, (data: any) => {
      if (!data) {
        this.push(null);
  ...
  term (plainTerm: Term) {
    switch (plainTerm.termType) {
      ...
      case 'Literal':
        return this.factory.literal(plainTerm.value, plainTerm.language ||
           this.factory.namedNode(plainTerm.datatype.value));
      case 'DefaultGraph':
  ...
  parse (data: any) {
    return ParserStream.toPlainObject(data).then((object) => {
      ...
      const toRdfOptions: any = { base: this.baseIRI };
      ...
      plainQuads.forEach((plainQuad: Quad) => {
