...
import { quadWith1, quadWithPredicateObject2, quadWithPredicate2,
  quadWithLabelLiteral, quadWithDefintionLiteral, quadWithBroaderConcept, quadWithRelatedConcept } from '../TestData';
...
describe('<ConceptList />', () => {
  ...
  it('shows a broader concept', () => {
    const wrapper = shallow(<ConceptList
      ...
      />);
    expect(wrapper.find({ href: quadWithBroaderConcept.object.value })
  ...
  it('shows a related concept', () => {
    const wrapper = shallow(<ConceptList
      ...
      />);
    expect(wrapper.find({ href: quadWithRelatedConcept.object.value })
    .getElements().length).toBeGreaterThan(0);
  });
});
