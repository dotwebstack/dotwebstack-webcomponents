...
import {
  objectTest1, quadTree1, quadTree2, quadTree3, quadTree4, quadTree5, quadTreeLoop5,
  quadTreeLoop6, quadTreeLoop7, quadTreeLoop8, objectTest2,
} from '../TestData';
...
describe('<PropertyTree />', () => {
  ...
  it('shows nested tree, grandchildren collapsed', () => {
    ...
    expect(wrapper.find({ href: quadTree2.object.value }).getElements().length)
      .toBeGreaterThan(0);
    expect(wrapper.find({ href: quadTree3.object.value }).getElements().length)
      .toBeGreaterThan(0);
  });
  ...
  it('shows multiple unlinked trees', () => {
    ...
    expect(wrapper.find({ href: quadTree5.object.value }).getElements().length)
      .toBeGreaterThan(0);
    expect(wrapper.find({ href: quadTree4.object.value }).getElements().length)
      .toBeGreaterThan(0);
    expect(wrapper.find({ href: quadTree1.object.value }).getElements().length)
      .toBeGreaterThan(0);
  });
  ...
  it('handles multiple property IRIs', () => {
    const wrapper = mount(
      <PropertyTree
        propertyIris={[objectTest1, objectTest2]}
